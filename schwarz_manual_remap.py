...
for path in old_run_dir.glob("*.nc"):
    if "_ATMIFS_" in path.stem:
        ...
        if oce_var_name is None:
            continue
        oce_file_path_tmp = run_dir / f"{oce_var_name}_tmp.nc"
        oce_file_path = run_dir / f"{oce_var_name}.nc"
    ...
    if "_oceanx_" in path.stem:
        ...
        if atm_var_name is None:
            continue
        atm_file_path_tmp = run_dir / f"{atm_var_name}_tmp.nc"
        atm_file_path = run_dir / f"{atm_var_name}.nc"
